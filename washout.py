...
conf["target_slow_colour"]    = (50,50,50)
conf["target_correct_colour"] = (50,50,50)
conf["target_fast_colour"]    = (50,50,50)
...
# Cutoffs on the time interval for determining whether the movement was too slow
conf['too_slow_time']= 1   
conf['too_fast_time']= 0.8

# start and end time to calculate the time interval of movement for giving speed feedback
# these are just initial values and their value is calculated during movement on each trial
conf['tend']= 0 
conf['tstart']= 0
...
# The cursor colour
conf["active_cursor_colour"]  = (255,0,0)  #(25,90,25)  # (0,255,0)
...
conf['robot_center']= (-0.02,-.06) # robot center position
...
conf['review_target_slow_colour']     = (0, 0, 255)
conf['review_target_fast_colour']     = (255, 0, 0)
conf['review_target_correct_colour']  = (0, 255, 0)
...
conf['recognition_display_point']= False
conf['recognition_display_expanding_arc'] = True
...
def start_new_trial():
    ...
    if trialdata['current_schedule']==-1:
...
def get_target_sound():
    tspan = conf['tend'] - conf['tstart']
    if not tspan: # if we don't have a reading
        return conf["target_slow_tone"]

    if tspan>conf['too_slow_time']:
        return conf["target_slow_tone"]
    if tspan<conf['too_fast_time']:
...
def get_target_colour_review():
    
    tspan = conf['tend'] - conf['tstart']

    if not tspan: # if we don't have a reading
        return conf["review_target_slow_colour"]

    if tspan>conf['too_slow_time']:
        return conf['review_target_slow_colour']
    if tspan<conf['too_fast_time']:
        return conf['review_target_fast_colour']
    return conf['review_target_correct_colour']
...
def mainloop():
    ...
    while gui['keep_going']:
        ...
        if phase_is('move'):
            ...
            sx,sy = conf['robot_center']
            x_tmp,y_tmp = robot.rshm('x'),robot.rshm('y')
            if flgstart == False and math.sqrt(math.pow(x_tmp-sx, 2)+math.pow(y_tmp-sy, 2)) >= conf['center_marker_radius']:
                conf['tstart'] = trialdata['t.absolute']
                flgstart = True
            if flgend == False and robot.rshm('fvv_move_done'):
                conf['tend'] = trialdata['t.absolute']
                flgend = True
        ...
        if trialdata['redraw']:
            ...
            if 'target_position' in trialdata and not trialdata['type']=='pinpoint': # We show a target always, only not if this is a pinpoint trial
                ...
                elif schedule and schedule.get('target.type',None)=='arcpoint':
                    draw_arc(conf['screen'],conf['movement_radius'],col)
                    draw_ball(conf['screen'],trialdata['target_position'],conf['target_radius'],col)
            ...
            if phase_in(['forward','stay','fade','move']):
                ...
                if trialdata['type'] in ['passive','active']:
                    ...
                    if (not conf['na.cursor.show']) and (clamp_trial or noexparc_trial): # shahryar - changed
                        ...
                        showcursor = (trialdata['type']=='active') and sal
...
def init_logs():
    ...
    basedir = './data/washout/%s'%conf['participant']
...
def read_schedule_file():
    ...
    for tt in ttypes:
        if tt not in ['arc','point', 'arcpoint', 'none']:
...
def recognition_display(trialdata,markersonly=False):
    ...
    elif conf['recognition_display_point']:
    ...
    if not markersonly: # if we also show a marker indicating the subject position somehow (could be a bar or arc or cursor)
        trialdata['bar_position'],dist=visual_error_clamp( (trialdata['robot_x'],trialdata['robot_y']), conf['robot_center'], trialdata['target_position'] )
        if conf['recognition_display_expanding_arc']:
